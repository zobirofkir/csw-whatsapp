...
export default function Post({ post }: PostProps) {
    ...
    const handleReaction = async (type: string) => {
        try {
            const currentReaction = states.userReaction;
            const currentCounts = { ...states.reactionCounts };

            if (currentReaction === type) {
                states.setUserReaction(undefined);
                states.setReactionCounts((prev) => ({
                    ...prev,
            ...
            states.setReactionCounts(response.data.reactionCounts);
            states.setUserReaction(response.data.userReaction);
            states.setShowReactionPicker(false);
        } catch (error) {
            console.error('Error setting reaction:', error);
            states.setReactionCounts(currentCounts);
            states.setUserReaction(currentReaction);
        }
    };

    const handleComment = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!states.newComment.trim() || states.isSubmitting) return;

        states.setIsSubmitting(true);
        try {
            const response = await axios.post(`/posts/${post.id}/comment`, {
                content: states.newComment,
            });
            states.setComments([response.data.comment, ...states.comments]);
            states.setNewComment('');
        } catch (error) {
    ...
    const handleCommentReaction = async (commentId: number) => {
        try {
            states.setLoadingCommentReaction(commentId);
            states.setComments(
            ...
            states.setLoadingCommentReaction(null);
        }
    };

    const handleReply = async (commentId: number) => {
        const replyContent = states.replyContents[commentId];
        if (!replyContent?.trim() || states.isSubmitting) return;

        states.setSendingReply(commentId);
        states.setIsSubmitting(true);
        ...
        if (navigator.share) {
            try {
                ...
                if ((error as Error).name !== 'AbortError') {
                    states.setShowShareDialog(true);
                }
            }
        } else {
            states.setShowShareDialog(true);
    ...
    const copyToClipboard = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            states.setShowShareDialog(false);
        } catch (error) {
